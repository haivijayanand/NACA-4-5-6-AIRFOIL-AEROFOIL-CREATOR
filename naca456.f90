...
IMPLICIT NONE
  ...
  INTEGER,PARAMETER:: IN=1, OUT=2, DBG=3, GNU=4, OUT1=5  ! unit assignments
...
! NOTE - dateTimeStr and all file numbers are global variables
  ...
  INTEGER:: errCode,Len_Str
  CHARACTER(LEN=132):: fileName,fileName1,fileName2,fileName3
...
!-------------------------------------------------------------------------------
  ...
  Len_Str = LEN(Trim(fileName))
  WRITE(fileName1,'(A,".dbg")')Trim(fileName(1:Len_Str-4))
  WRITE(fileName2,'(A,".out")')Trim(fileName(1:Len_Str-4))
  WRITE(fileName3,'(A,".gnu")')Trim(fileName(1:Len_Str-4))
  
  OPEN(UNIT=DBG, FILE=Trim(fileName1), STATUS='REPLACE', ACTION='WRITE')
  WRITE(DBG,*) GREETING
  WRITE(DBG,*) "Created by naca456, version "//VERSION//" on "//dateTimeStr
  WRITE(DBG,*) AUTHOR

  OPEN(UNIT=OUT, FILE=Trim(fileName2), STATUS='REPLACE', ACTION='WRITE')
  WRITE(OUT,*) GREETING
  WRITE(OUT,*) "Created by naca456, version "//VERSION//" on "//dateTimeStr
  WRITE(OUT,*) AUTHOR

  OPEN(UNIT=GNU, FILE=Trim(fileName3), STATUS='REPLACE', ACTION='WRITE')
